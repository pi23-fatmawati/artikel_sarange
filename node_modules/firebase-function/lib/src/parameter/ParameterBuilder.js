...
(function (ParameterBuilder) {
    ...
    function nullable(builder) {
        const expectedTypes = builder.expectedTypes.includes('object') ? builder.expectedTypes : ['object', ...builder.expectedTypes];
        return new ParameterBuilder(expectedTypes, (value, logger) => {
            logger.log('ParameterBuilder.nullable', { expectedTypes: builder.expectedTypes, value: value });
            if (value === null)
                return null;
            if (typeof value === 'object' && !builder.expectedTypes.includes('object'))
                throw (0, HttpsError_1.HttpsError)('invalid-argument', `Type of value is object, but expected null or ${builder.expectedTypes}`, logger);
            return builder.build(value, logger.nextIndent);
        });
    }
    ParameterBuilder.nullable = nullable;
